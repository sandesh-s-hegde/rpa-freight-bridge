...
class BackgroundTaskRegistry:
    ...
    async def wait_all(self, timeout: float = 25.0) -> None:
        if self.tasks:
            logger.info(f"Graceful shutdown: Waiting for {len(self.tasks)} in-flight background tasks...")
            done, pending = await asyncio.wait(self.tasks, timeout=timeout)
            if pending:
                logger.warning(f"Shutdown timeout reached. {len(pending)} tasks forcefully terminated.")
            else:
