        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if self.state == "HALF_OPEN":
                    self.state = "CLOSED"
                    self.failure_count = 0
                    logger.info("Circuit Breaker: Recovered and transitioned to CLOSED state.")
                return result
                ...
                if self.failure_count >= self.failure_threshold:
                    self.state = "OPEN"
                    logger.error("Circuit Breaker: Tripped to OPEN state due to consecutive upstream failures.")
                raise e
        return wrapper
